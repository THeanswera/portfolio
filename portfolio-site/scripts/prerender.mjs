 ...
 * ошибки — её отдаёт адрес любой глубины (/nope, /a/b/c), и './favicon.svg'
 * искал бы файл рядом с ним. Поэтому у страниц в корне (главная и 404) адрес
 * от документа './x' заменяется на путь от корня '/x', а у вложенных страниц
 * и './x', и абсолютный '/x' — на путь от их папки '../x': так одна и та же
 * разметка не зависит от того, по какому адресу её открыли.
 */
function makeRefsRelative(html, up) {
  const fromRoot = html.replace(/(\b(?:href|src)=")\.\/(?!\.)/g, `$1${up || '/'}`);

  if (!up) return fromRoot;
