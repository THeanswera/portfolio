...
function footer() {
  ...
  // Пометка о демонстрационном проекте. Раньше она была только в микроразметке
  // для поисковика: посетитель видел цены и телефон, но не знал, что компания
  // вымышлена. Флаг `demo` стоит в данных сайта — при реальном заказчике он
  // снимается вместе с реквизитами, и пометка исчезает сама.
  const demoNotice = site.demo
    ? `<p class="footer__demo">
      ${icon('info', { size: 18 })}
      <span>
        Демонстрационный проект: компания, адрес, телефон и цены вымышлены. Услуги не оказываются,
        формы показывают пример интерфейса и никуда не отправляют данные. Расчёты конфигуратора —
        образец работы с ценой, а не коммерческое предложение.
      </span>
    </p>`
    : '';
